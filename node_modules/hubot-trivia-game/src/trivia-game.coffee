...
class Game
  ...
  apiGet: (model, id = '') ->
    console.log id
    @robot.http("#{@baseApiUrl}/#{model}/#{id}")
      .get() (err, res, body) ->
        if err
          return err
        console.log(body)
  ...
  dev: (resp, command, args...) ->
    switch command
      when "list-quizzes" then @apiGet 'quizzes'
      when "get-quiz" then @apiGet 'quizzes', args[0]
      when "create-quiz" then @apiPost 'quizzes/create', { "quiz-name": args[0] }
      when "add-question-to-quiz" then @addQuestion args[0]
