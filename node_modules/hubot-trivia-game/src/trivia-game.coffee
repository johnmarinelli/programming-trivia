...
class ApiClient
  ...
  dev: (resp, command, args...) ->
    ...
    switch command
      ...
      when "get-quiz" then response = @apiGet('quizzes', listQuizzesCallback, {id: args[0]})
      ...
      when "remove-question-from-quiz" then response = @removeQuestion args[0], args[1]
      when "remove-quiz" then response = @removeQuiz args[0]
      else resp.send "Command '#{command}' not found."
...
module.exports = (robot) ->
  ...
  robot.hear /^!dev ([A-Za-z-_]+)\s?([A-Za-z-_0-9]+)?\s?([A-Za-z-_0-9]+)?/, (resp) ->
